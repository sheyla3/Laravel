...
<body>
    ...
    <div class="container">
        <h1>Afegir Casal</h1>
        <form action="{{ route('casal.editar', $casal->id)}}" method="POST" class="mb-4" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
                <label for="nom" class="form-label">Nom:</label>
                <input type="text" id="nom" name="nom" value="{{ $casal->nom ?? '' }}"  class="form-control">
                {!! $errors->first('nom', '<small class="text-danger">:message</small>') !!}
            </div>
            <div class="mb-3">
                <label for="data_inici" class="form-label">Data de inici:</label>
                <input type="date" id="data_inici" name="data_inici" min="{{ $fechaActual }}" value="{{ isset($casal->data_inici) ? \Carbon\Carbon::parse($casal->data_inici)->format('Y-m-d') : '' }}"
                    class="form-control">
                {!! $errors->first('data_inici', '<small class="text-danger">:message</small>') !!}
            </div>
            <div class="mb-3">
                <label for="data_final" class="form-label">Data final:</label>
                <input type="date" id="data_final" name="data_final" min="{{ $fechaDias }}" value="{{ isset($casal->data_final) ? \Carbon\Carbon::parse($casal->data_final)->format('Y-m-d') : '' }}"
                    class="form-control">
